...
```{r}
packages <- c("randomForest","dplyr", "ggplot2", "caret", "Amelia", "tidyr","pROC", "parallel") #, "Zelig", ) #If amelia is used Betadist and mi can be removed
...
imputer <-function(data,imputations =5, number_cols = 119,stockspergroup = 100  )  {
    #reformat for time series imputations
    datatime <- data %>% select(Ret_2:Ret_120) %>% t %>% data.frame %>% 
...
#ImputedTrainTime <- imputer(data, imputations =5, number_cols ,stockspergroup )

#testImput <- imputer(data = test, imputations =5, number_cols ,stockspergroup )

#create a list of the imputed time series and the unimputed feature set

Imputtest <- imputer(data[101:200,], imputations =5, number_cols ,stockspergroup=20 )

rowSums(Imputtest[[1]] == data[101:200,29:147], na.rm=TRUE) #this si a problem

x <-data.frame(alphaordering= paste("X",1:40000, sep="")) %>% arrange(alphaordering) %>%
  mutate(alphaordering = (sub("X","",alphaordering)))

testimput <- ImputedTrainTime[[1]]
testimput2 <- testimput %>% bind_cols(x) %>% arrange(alphaordering) %>% select(-alphaordering)

rowSums(testimput2[1:5,1:5] == data[1:5,29:33], na.rm=TRUE)
...
y <- data[1:10,29:147]
y
rowSums(ImputedTrainTime[[1]][1:5,] == data[1:5,29:147], na.rm=TRUE)
