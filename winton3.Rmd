...
```{r}
packages <- c("randomForest","dplyr", "ggplot2", "caret", "Amelia", "tidyr","pROC", "parallel") #, "Zelig", "doMC") #If amelia is used Betadist and mi can be removed
...
base line predictive power
```{r}
#Calculate RMSE for 0, the RMSE is 0.0250393
postResample(rep(0,nrow(data)), data$Ret_PlusOne)

#Calculate RMSE using median, slightly better than using 0
medval <- rep(median(responseOneTrain), length(data$Ret_PlusOne[-trainIndex]))
postResample(medval,data$Ret_PlusOne[-trainIndex])

#Calculate RMSE for predicting the sign and putting in the median value RMSE it is better than zero prediction
signmeds<- rep(NA, length(data[-trainIndex,]))
signmeds <-ifelse(data$Ret_PlusOne[-trainIndex] >0,MedTrainPos,MedTrainNeg)
postResample(signmeds, data$Ret_PlusOne[-trainIndex])
...
z <- sapply(seq(0.5,to = 1, by = .05), function(n){
    x <- rep(NA, 30)
      for (i in 1:30){
        ...
        x[i] <- postResample(testsignmeds, data$Ret_PlusOne[-trainIndex])[1]
      }
    x
  }
)

# At 70% correct identification it is better to classify than use 0
rbind(seq(0.5,to = 1, by = .05), colMeans(z))
...
Create a list of models to predict the Plus one return
```{r}

datlength <- 3# length(datlist2)

RegNNlistPlusOne <- vector("list", datlength )

RegNNlistPlusOneCat <- vector("list", datlength )

for (n in 1:datlength){
  print(paste("Regress", n, "of", datlength))
    RegNNlistPlusOne[[n]] <-train(x = datlist2[[i]][trainIndex,-1],
...
for (n in 1:datlength){
  print(paste("catag", n, "of", datlength))
    RegNNlistPlusOneCat[[n]] <-train(x = datlist2[[i]][trainIndex,-1],
                              y = data$Ret_PlusOne[trainIndex],  
                            method = "pcaNNet",
                            trControl = train_control)
}
...
RegNNlistPlusOnex <-foreach (i in 1:datlength) %dopar% {
  #print(paste(n, "of", length(datlist2)))
   train(x = datlist2[[i]][trainIndex,-1],y = data$Ret_PlusOne[trainIndex],  
                        method = "pcaNNet",
                        trControl = train_control)
}
...
for (i in 1:datlength){
  #print(paste(n, "of", length(datlist2)))
  RegNNlistPlusOneCat[[i]] <- train(x = datlist2[[i]][trainIndex,-1],y = PosNegcat[trainIndex],  
                        method = "pcaNNet",
                        trControl = train_control)
}
...
nnetTuneGridReg1 <- train(x = datlist2[[1]][trainIndex,-c(1, highcor)],
                  y = responseOneTrain <- data$Ret_PlusOne[trainIndex],
                  method = "avNNet",
                  tuneGrid = nnetGrid,
                  trControl = train_control,
                  preProc = c("center", "scale"),
...
x2 <- train(x = datlist2[[1]][trainIndex,-1],y = PosNegcat[trainIndex],  
                         method = "pcaNNet", 
                         trControl = train_control)

xx <- sapply(1:2, function(n){
    predict(RegNNlistPlusOne[[n]], datlist2[[n]])
})  



#Calculates RMSE for a regression model
postResample(predict(x, datlist2[[1]][-trainIndex,]), data$Ret_PlusOne[-trainIndex])


#binds a list of models for comparison and aggregation
z <- cbind(xx, rowMeans(xx))

sapply(1:ncol(z), function(n) postResample(z[,n], data$Ret_PlusOne))

posnegpred <- predict(nnetTune, datlist2[[1]][-trainIndex,])
confusionMatrix(posnegpred, PosNegcat[-trainIndex])


# Calculate RMSE for classify sign then insert positive or negative median
signmeds2<- rep(NA, length(posnegpred))
signmeds2[PosNegcat[-trainIndex] ==1] <- median(responseOneTrain[PosNegcat[trainIndex] ==1])
signmeds2[PosNegcat[-trainIndex] ==1] <- median(responseOneTrain[PosNegcat[trainIndex] !=1])

postResample(signmeds2, data$Ret_PlusOne[-trainIndex])
...
classPosNeg <- train(x = datlist2[[1]][,-1],y = make.names(PosNegcat),  
                    method = "pcaNNet",
                    nbagg = 50,
                    metric = "ROC",
                    trControl = train_control)
...
RegNNlistPlusOne <-lapply(1:2, function(n){
                    print(paste(n, "of", length(datlist2)))
#                     train(x = n[,-1],y = data$Ret_PlusOne,  
#                         method = "pcaNNet",
#                         trControl = train_control)
      }
  )
