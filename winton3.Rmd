...
data2 <- data[25:30,-c(lowvar, highNAS,29:211)] %>% mutate(Id = paste("X",Id, sep=""))

#reformat for time series imputations
datatime <- data[25:30,] %>% select(Ret_2:Ret_120) %>% t %>% data.frame %>% 
...
imputs3 <- datatime %>% amelia(., ts="timeID", cs = "Id", 
                                      polytime = 2, intercs = TRUE, m= 20)

tscsPlot(imputs3, cs ="X26", var = "ROI")
#recombines the data with the feature data and the timeseries descriptive statistics
PreFeatureImp <- vector("list", length(imputs3$imputations))

for (i in 1:length(imputs3$imputations))  {
...
for (i in 1:length(ameliaList))  {
  ameliaList <- PreFeatureImp[[i]] %>% select(Feature_3:gm_mean) %>% amelia(m= 5)
