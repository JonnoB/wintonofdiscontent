...
# error on using the known target row timeseries average, it is marginally better as 0.0006290278
rowmeds <-  sapply(ImputedTrainTime, function(n) {
              apply(n,1,median) #%>% rep( each = 60)
    }
  ) %>% apply(.,1,median)
...
#the previous neural network returned 10 hidden layers and a decay of 0.5, hopefully by imputing across all the data sets and averaging the results I will get a better result than if I just use one!
nnetGrid <- expand.grid(.decay=c(0.05),
                        .size = c(10), .bag = FALSE)
...
#This model uses an averaged neural net to regress return
RegNNlistPlusOne[[i]] <- train(x = datlist2[[i]][trainIndex,-c(1, highcor)],
                          y = responseOneTrain <- data$Ret_PlusOne[trainIndex],
                          method = "avNNet",
                          tuneGrid = nnetGrid,
                          trControl = train_control,
